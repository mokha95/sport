...
// l utilisateur peut ajouter un space
...
function AddEditSpace(props) {
  ...
  if (space) {
    formOptions.defaultValues = props.space;
  }
  ...
  async function onSubmit(data) {
    alertService.clear();
    try {
      // create or update space based on space prop
      let message;
      if (space) {
        await spaceService.update(space.id, data);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="mb-3">
        ...
        <Link href="/espaces" className="btn btn-link">
          Annuler
        </Link>
      </div>
    </form>
